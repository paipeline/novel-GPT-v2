...
class Agents:
    ...
    def create_agents(self):
        title_generator = Agent(
            role='书名生成器',
            goal='1.根据小说的大纲生成一个吸引人的书名。2.与outline_designer和character_designer合作，确保书名与故事的主题和角色一致。3.书名应简洁明了，不超过六个字。',
            verbose=True,
            memory=True,
        ...
        chapter_writer = Agent(
            ...
            verbose=True,
            memory=True,
            backstory="一个以详细的叙事和复杂的角色和场景开发而闻名的熟练小说家。",
            tools=[],
            llm=self.llm4o,
            allow_delegation=False
        )
        ...
        outline_designer = Agent(
            role='大纲设计师',
            goal='1. 为每一章制定一个全面的大纲，每章必须有一个对整体情节有贡献的目标和高潮点。 \
        ...
        supervising_editor = Agent(
            role='监督编辑',
            goal='1. 监督小说的发展，以确保从大纲到最终手稿的质量和连贯性。 \
                2. 检查整体风格及其与叙述基调的一致性。 \
        ...
        scene_designer = Agent(
            role='场景设计师',
            goal='1. 开发详细且引人入胜的场景描述，为小说的动作设置舞台。 \
